...
import { Plus as Agregar, Search as Buscar, RefreshCw as Actualizar, Pencil as Editar, Users as IconoModulo, CircleAlert as Alerta, X as Cerrar } from '@lucide/vue'
...
const filtrados = computed(() => {
  ...
  return registros.value.filter(registro =>
    ...
    [registro.nombre, registro.razon_social, registro.identificacion, registro.codigo, registro.nombre_municipio, registro.telefono, registro.correo]
      .some(valor => String(valor ?? '').toLocaleLowerCase('es').includes(busqueda)))
})
...
async function cargar() {
  ...
  error.value = ''
  try {
    const { data: respuesta } = await api.get('index.php/mnt/cliente/buscar')
    if (!Array.isArray(respuesta.lista)) throw new Error(respuesta.mensaje || 'No se pudo obtener el listado de clientes.')
...
<template>
  <div class="maintenance-screen">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div class="space-y-3">
        <RutaNavegacion :items="[{ label: 'Mantenimiento' }, { label: 'Clientes' }]" />
        <h2 class="flex items-center gap-2 text-xl font-semibold leading-tight tracking-tight"><IconoModulo :size="22" class="text-accent" aria-hidden="true" /> Clientes</h2>
      </div>
    ...
    <TarjetaBase no-padding class="maintenance-catalog">
      <div class="flex flex-col gap-2 border-b border-line px-4 py-2 sm:px-5 xl:flex-row xl:items-center xl:justify-between xl:gap-4">
        <div class="flex max-w-full flex-wrap gap-1" role="group" aria-label="Filtrar clientes por estado">
        ...
        <div class="flex min-w-0 items-center gap-2 xl:w-96 xl:shrink-0">
          <div class="relative min-w-0 flex-1">
            <label for="buscar-cliente" class="sr-only">Buscar clientes</label>
            <Buscar :size="16" class="pointer-events-none absolute left-3 top-3 text-muted" aria-hidden="true" />
            <input id="buscar-cliente" v-model="termino" type="search" class="min-h-10 w-full rounded-lg border border-line bg-canvas py-2 pl-9 pr-3 text-sm text-ink placeholder:text-muted focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10" placeholder="Buscar cliente…" />
          </div>
          ...
          <button type="button" class="inline-flex size-10 shrink-0 items-center justify-center rounded-lg text-muted transition-colors hover:bg-soft hover:text-ink" aria-label="Actualizar clientes" title="Actualizar clientes" :disabled="cargando" @click="cargar">
      ...
      <div v-else-if="error" class="flex flex-col items-center gap-3 px-6 py-12 text-center" role="alert">
        ...
        <h3 class="font-semibold">No pudimos cargar los clientes</h3>
      ...
      <template v-else>
        <TablaBase v-if="filtrados.length" :columns="columnas" :rows="filtrados" class="[&_th]:px-3! [&_th]:py-1.5! [&_th]:text-xs [&_td]:px-3! [&_td]:py-1! [&_td]:text-[13px]">
          ...
          <template #cell-credito="{ value: credito }">
            <span class="text-xs font-medium" :class="Number(credito) === 1 ? 'text-accent' : 'text-muted'">{{ Number(credito) === 1 ? 'Sí' : 'No' }}</span>
          </template>
        ...
        <div v-else class="flex flex-col items-center px-6 py-14 text-center">
          ...
          <p class="mt-2 max-w-sm text-sm text-muted">{{ registros.length ? 'Prueba con otro nombre, identificación o código.' : 'Agrega tu primer cliente para tenerlo disponible en ventas.' }}</p>
          <button type="button" class="mt-5 min-h-11 rounded-xl border border-line px-5 text-sm font-semibold text-accent hover:bg-accent/5" @click="registros.length ? limpiarFiltros() : abrirFormulario()">{{ registros.length ? 'Limpiar filtros' : 'Agregar primer cliente' }}</button>
        </div>
        <div class="flex flex-wrap items-center justify-between gap-2 border-t border-line bg-soft/30 px-4 py-3 text-xs text-muted" aria-live="polite">
          <span><strong class="font-semibold text-ink">{{ filtrados.length }}</strong> de {{ registros.length }} clientes</span>
          <span>Selecciona un cliente para editarlo</span>
        </div>
      </template>
    </TarjetaBase>

    <FormularioRegistro v-if="mostrarFormulario" :registro="seleccionado" @cerrar="mostrarFormulario = false" @guardada="actualizarLista" />
  </div>
</template>
