...
import { X as Cerrar, Truck as IconoModulo, Check as Confirmar, LoaderCircle as Cargando } from '@lucide/vue'
...
async function cargarCatalogos() {
  ...
  try {
    const { data } = await api.get('index.php/mnt/proveedor/get_datos')
    if (!data.cat || ![].every(clave => Array.isArray(data.cat[clave]))) throw new Error('No se pudieron cargar los catálogos.')
...
async function guardar() {
  ...
  try {
    ...
    const { data: respuesta } = await api.post(`index.php/mnt/proveedor/guardar/${encodeURIComponent(id)}`, datos)
    ...
    <form :aria-busy="guardando" @submit.prevent="guardar">
      <header class="flex items-center justify-between gap-3 border-b border-line px-5 py-3">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 id="titulo-registro" class="text-lg font-semibold tracking-tight">{{ registro ? 'Editar proveedor' : 'Nuevo proveedor' }}</h2>
          </div>
        </div>
      ...
      <div class="space-y-4 p-5">
        ...
        <fieldset :disabled="guardando || cargandoCatalogos || errorCatalogos" class="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2">
          <legend class="sr-only">Información de proveedor</legend>
