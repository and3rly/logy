<template>
	<div class="d-flex align-items-center mb-3">
		...
		<div class="ms-auto">
			<a href="#" class="btn btn-theme" @click.prevent="frmRol(null)">
				...
				<table class="table table-sm table-hover text-nowrap m-0">
					...
					<tbody>
						...
						<tr v-if="filtrada.length === 0 && !cargando">
							<td class="text-center py-2" colspan="100">
						...
						>
							...
							<td class="fw-bold">
								<a href="javascript:;" class="text-decoration-none" @click="frmRol(i)">
									{{ i.nombre }}
								</a>
							</td>
	...
	<div
		...
		tabindex="-1"
		aria-labelledby="tituloMdlRol"
		aria-hidden="true"
	>
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="tituloMdlRol">
...
<script>
	import Form from "@/views/mnt/rol/Form.vue"
	...
	export default {
		...
		methods: {
			buscar() {
				...
				this.$http
					.get(`${this.$baseUrl}/mnt/rol/buscar`, {params: this.bform})
					.then(res => {
			...
			frmRol(obj) {
				this.verForm = true
				this.rol = obj
				this.$abrirModal("mdlRol")
			},
			cerrarFrm() {
				this.verForm = false
				this.rol = null
				this.$cerrarModal("mdlRol")
			},
			actualizarLista(obj) {
				if (this.rol === null) {
			...
			filtrada() {
				...
				return this.lista.filter(obj =>
					Object.values(obj).some(valor =>
						valor != null && String(valor).toLowerCase().includes(termino)
					)
				)
			}
		},
