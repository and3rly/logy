<template>
	<div class="d-flex align-items-center mb-3">
		...
		<div class="ms-auto">
			<a href="#" class="btn btn-theme" @click="frmMarca(null)">
				...
				<table class="table table-sm table-hover text-nowrap m-0">
					...
					<tbody>
						...
						>
							...
							<td class="fw-bold">
								<a href="javascript:;" class="text-decoration-none" @click="frmMarca(i)">{{ i.nombre }}</a>
							</td>
...
<script>
	import Form from "@/views/mnt/marca/Form.vue"
	...
	export default {
		...
		methods: {
			buscar() {
				...
				this.$http
				.get(`${this.$baseUrl}/mnt/marca/buscar`, {params: this.bform})
				.then(res => {
					this.cargando = false
			...
			frmMarca(obj) {
				this.verForm = true
				this.marca = obj
				this.$abrirModal("mdlMarca")
			},
			cerrarFrm() {
				this.verForm = false
				this.marca = null
				this.$cerrarModal("mdlMarca")
			},
			actualizarLista(obj) {
				if (this.marca === null) {
