...
import { X as Cerrar, Tag as IconoModulo, Check as Confirmar, LoaderCircle as Cargando } from '@lucide/vue'
...
async function guardar() {
  ...
  try {
    ...
    const { data: respuesta } = await api.post(`index.php/mnt/marca/guardar/${encodeURIComponent(id)}`, datos)
    if (Number(respuesta.exito) !== 1 || !respuesta.linea?.id) {
      throw new Error(respuesta.mensaje || 'No se pudo guardar la marca.')
    }
    ...
    <form :aria-busy="guardando" @submit.prevent="guardar">
      <header class="flex items-center justify-between gap-3 border-b border-line px-5 py-3">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 id="titulo-registro" class="text-lg font-semibold tracking-tight">{{ registro ? 'Editar marca' : 'Nueva marca' }}</h2>
          </div>
        </div>
      ...
      <div class="space-y-4 p-5">
        <fieldset :disabled="guardando" class="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2">
          <legend class="sr-only">Información de la marca</legend>
          <div class="sm:col-span-2">
            <label for="registro-nombre" class="mb-2 block text-sm font-semibold">Nombre <span class="text-accent">*</span></label>
            <input id="registro-nombre" v-model="formulario.nombre" class="min-h-10 w-full rounded-lg border border-line bg-surface px-3 py-2 text-sm text-ink placeholder:text-muted focus:border-accent focus:outline-none focus:ring-4 focus:ring-accent/10" placeholder="Ej. Marca" required autofocus />
          </div>
