<template>
	<div class="d-flex align-items-center mb-3">
		...
		<div class="ms-auto">
			<a href="#" class="btn btn-theme" @click="frmMoneda(null)">
				...
				<table class="table table-sm table-hover text-nowrap m-0">
					...
					<tbody>
						...
						>
							...
							<td class="fw-bold">
								<a href="javascript:;" class="text-decoration-none" @click="frmMoneda(i)">{{ i.nombre }}</a>
							</td>
...
<script>
	import Form from "@/views/mnt/moneda/Form.vue"
	...
	export default {
		...
		methods: {
			buscar() {
				...
				this.$http
				.get(`${this.$baseUrl}/mnt/moneda/buscar`, {params: this.bform})
				.then(res => {
					this.cargando = false
			...
			frmMoneda(obj) {
				this.verForm = true
				this.moneda = obj
				this.$abrirModal("mdlMoneda")
			},
			cerrarFrm() {
				this.verForm = false
				this.moneda = null
				this.$cerrarModal("mdlMoneda")
			},
			actualizarLista(obj) {
				if (this.moneda === null) {
					this.lista.push(obj)
				} else {
					for (let i in this.moneda) {
						this.moneda[i] = obj[i]
					}
				}
