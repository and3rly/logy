...
class Cuenta_cobrar_pago_model extends General_model {
	...
	public function _buscar($args=[])
	{
		...
		if (elemento($args, "cuenta_cobrar_id")) $this->db->where("a.cuenta_cobrar_id", $args["cuenta_cobrar_id"]);

		$tmp = $this->db->select("a.*, b.nombre as nombre_forma_pago,
			c.nombre as nombre_usuario, d.factura_numero, d.cliente_id, d.moneda_id,
			e.nombre as nombre_cliente, e.identificacion,
			...
			->join("cuenta_cobrar d", "d.id = a.cuenta_cobrar_id")
			->join("cliente e", "e.id = d.cliente_id AND e.empresa_id = d.empresa_id")
	...
	public function asignarRecibo($id, $fecha)
	{
		$numero = "REC-" . date("Y", strtotime($fecha)) . "-" . str_pad($id, 6, "0", STR_PAD_LEFT);
		$this->db->where("id", $id)->where("recibo_numero IS NULL", null, false)
			->update($this->_tabla, ["recibo_numero" => $numero]);
